...
function addUser(socket: Websocket, { meetingId, userId, name, profileImageUrl }: AddUserOptions): void {
  ...
  if (meetingUser) {
    ...
    meetingUsers.push({ socket, userId, joined: true, name, profileImageUrl, isAlive: true });
  }
}
...
function joinMeeting(meetingId: string, socket: Websocket, payload: MessagePayload) {
  const { userId, name, profileImageUrl } = payload.data;
  console.log('User joined meeting', userId);
  if (isMeetingPresent(meetingId)) {
    addUser(socket, { meetingId, userId, name, profileImageUrl });
